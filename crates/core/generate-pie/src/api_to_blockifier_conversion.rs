...
fn invoke_v1_to_blockifier(
    ...
    let api_txn_real = starknet_api::executable_transaction::InvokeTransaction::create(api_tx, &chain_id).unwrap();
...
async fn declare_v0_to_blockifier(
    ...
    // )))
    let api_txn_real =
        starknet_api::executable_transaction::DeclareTransaction::create(api_tx, class_info, &chain_id).unwrap();
...
fn l1_handler_to_blockifier(
    ...
    let api_txn_real =
        starknet_api::executable_transaction::L1HandlerTransaction::create(api_tx, &chain_id, paid_fee_on_l1).unwrap();
...
fn deploy_account_v1_to_blockifier(
    ...
    let api_txn_real =
        starknet_api::executable_transaction::DeployAccountTransaction::create(api_tx, &chain_id).unwrap();
...
pub async fn starknet_rs_to_blockifier(
    ...
    let conversion_result = match sn_core_tx {
        ...
        Transaction::Declare(tx) => match tx {
            DeclareTransaction::V0(tx) => declare_v0_to_blockifier(tx, client, block_number, chain_id).await?,
            DeclareTransaction::V1(tx) => declare_v1_to_blockifier(tx, client, block_number, chain_id).await?,
            DeclareTransaction::V2(tx) => declare_v2_to_blockifier(tx, client, block_number, chain_id).await?,
            DeclareTransaction::V3(tx) => declare_v3_to_blockifier(tx, client, block_number, chain_id).await?,
        },
