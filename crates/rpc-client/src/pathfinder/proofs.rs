...
pub fn convert_storage_to_pathfinder_proof(storage_proof: StorageProof) -> PathfinderProof {
    ...
    for node in storage_proof.contracts_proof.nodes {
        contract_proof.push(convert_to_trie_node(&node));
    }

    let mut storage_proofs: Vec<Vec<TrieNode>> = Vec::new();
    for nodes in &storage_proof.contracts_storage_proofs {
        let mut trie_nodes: Vec<TrieNode> = Vec::new();
        for node in nodes {
            trie_nodes.push(convert_to_trie_node(node));
        }
        storage_proofs.push(trie_nodes);
    }

    let contract_data: Option<ContractData>;
    if storage_proof.contracts_storage_proofs[0].is_empty() {
        contract_data = None;
    } else {
        match storage_proof.contracts_storage_proofs[0][0].node {
            MerkleNode::Binary { left, right: _ } => {
                contract_data = if left.to_hex_string() == "0x0" {
                    Some(ContractData {
                        root: storage_proof.contracts_storage_proofs[0][0].node_hash,
                        storage_proofs: vec![],
                    })
                } else {
                    Some(ContractData { root: storage_proof.contracts_storage_proofs[0][0].node_hash, storage_proofs })
                }
            }
            MerkleNode::Edge { child: _, path: _, length: _ } => {
                contract_data =
                    Some(ContractData { root: storage_proof.contracts_storage_proofs[0][0].node_hash, storage_proofs })
            }
        };
    ...
    PathfinderProof {
        state_commitment: Some(storage_proof.global_roots.block_hash),
...
pub fn convert_storage_to_pathfinder_class_proof(storage_proof: StorageProof) -> PathfinderClassProof {
    ...
    for node in &storage_proof.classes_proof {
        class_proof.push(convert_to_trie_node(node));
    }
