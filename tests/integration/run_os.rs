...
async fn prepare_extensive_os_test_params(
    ...
    for (salt, calldata) in salts.into_iter().zip(calldatas.into_iter()) {
        ...
        let declare_tx = declare_tx(
            declare_tx_args! {
                ...
                class_hash: test_contract.class_hash,
                version: TransactionVersion::ONE,
    ...
    let delegate_proxy = initial_state.cairo0_contracts.get("delegate_proxy").unwrap();

    let delegate_proxy_address = delegate_proxy.address;

    let deploy_tx_args = deploy_account_tx_args! {
        class_hash: delegate_proxy.class_hash,
        version: TransactionVersion::ONE,
    };
    txs.push(Transaction::AccountTransaction(AccountTransaction::DeployAccount(deploy_account_tx(
        deploy_tx_args,
        nonce_manager,
    ))));
    ...
    let test_contract2 = initial_state.cairo0_contracts.get("account_with_long_validate").unwrap();
