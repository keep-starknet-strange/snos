...
pub fn is_block_number_in_block_hash_buffer(
    ...
    // TODO: above returns UnknownIdentifier("current_block_number"), seems like we have a cairo-vm issue here
    // Relevant cairo code is here: https://github.com/starkware-libs/cairo-lang/blob/efa9648f57568aad8f8a13fbf027d2de7c63c2c0/src/starkware/starknet/core/os/execution/execute_syscalls.cairo#L800
    // Hint reference in ids_data looks strange: HintReference { offset1: Value(0), offset2: Value(0), dereference: true, ap_tracking_data: None, cairo_type: Some("felt") }
    // while from compiled json file, it looks like it should be: [cast([[fp + (-3)] + 5], felt*)]
    ...
    let stored_block_hash_buffer = constants.get(STORED_BLOCK_HASH_BUFFER)
