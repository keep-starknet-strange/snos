...
pub const ASSIGN_BYTECODE_SEGMENTS: &str = indoc! {r#"bytecode_segments = iter(bytecode_segment_structure.segments)"#};

pub const COMPUTE_SLOPE: &str = indoc! {r#"from starkware.cairo.common.cairo_secp.secp256r1_utils import SECP256R1_ALPHA, SECP256R1_P
...
pub const ASSIGN_STATE_ENTRY_WITH_NEW_SEGMENT: &str = indoc! {r#"# Fetch a state_entry in this hint and validate it in the update that comes next.
ids.state_entry = __dict_manager.get_dict(ids.contract_state_changes)[ids.contract_address]
ids.new_state_entry = segments.add()"#};

pub const SET_STATE_ENTRY_TO_ACCOUNT_CONTRACT_ADDRESS: &str = indoc! {r#"# Fetch a state_entry in this hint and validate it in the update that comes next.
ids.state_entry = __dict_manager.get_dict(ids.contract_state_changes)[
    ids.tx_info.account_contract_address
]"#};

pub const CALCULATE_VALUE: &str = indoc! {r#"from starkware.cairo.common.cairo_secp.secp_utils import SECP256R1, pack
...
pub const COMPARE_RETURN_VALUE_2: &str = indoc! {r#"# Check that the actual return value matches the expected one.
...
pub const COMPUTE_NEW_Y: &str = indoc! {r#"value = new_y = (slope * (x - new_x) - y) % SECP256R1_P"#};

pub const COMPUTE_VALUE_DIV_MOD: &str = indoc! {r#"from starkware.python.math_utils import div_mod

value = div_mod(1, x, SECP256R1_P)"#};

pub const DATA_TO_HASH_NEW_SEGMENT: &str = indoc! {r#"ids.data_to_hash = segments.add()"#};
...
pub const ASSERT_END_OF_BYTECODE_SEGMENTS: &str = indoc! {r#"assert next(bytecode_segments, None) is None"#};

pub const ADDITIONAL_DATA_NEW_SEGMENT: &str = indoc! {r#"ids.additional_data = segments.add()"#};

pub const WRITE_ADDRESS: &str = indoc! {r#"from starkware.starknet.core.os.data_availability.bls_utils import split

segments.write_arg(ids.res.address_, split(ids.value))"#};

pub const COMPUTE_IDS_HIGH_LOW: &str = indoc! {r#"from starkware.cairo.common.math_utils import as_int
...
pub const WRITE_ZKG_COMMITMENT_ADDRESS: &str = indoc! {r#"execution_helper.store_da_segment(
...
pub const WRITE_CASE_NOT_LEFT_TO_AP: &str = indoc! {r#"memory[ap] = int(case != 'left')"#};

pub const EXIT_SYSCALL_GET_BLOCK_NUMBER: &str = indoc! {r#"exit_syscall(selector=ids.GET_BLOCK_NUMBER_SELECTOR)"#};

pub const ITER_CURRENT_SEGMENT_INFO: &str = indoc! {r#"current_segment_info = next(bytecode_segments)
...
pub const WRITE_TX_NONCE_TO_MEMORY: &str = indoc! {r#"memory[fp + 4] = to_felt_or_relocatable(tx.nonce)"#};

pub const MAYBE_WRITE_ADDRESS_TO_AP: &str = indoc! {r#"memory[ap] = to_felt_or_relocatable(ids.response.ec_point.address_ if ids.not_on_curve == 0 else segments.add())"#};

pub const GENERATE_NIBBLES: &str = indoc! {r#"num = (ids.scalar.high << 128) + ids.scalar.low
...
pub const IS_ON_CURVE_2: &str = indoc! {r#"ids.is_on_curve = (y * y) % SECP256R1.prime == y_square_int"#};

// TODO: looks similar to PACK_X_PRIME (above)
pub const PACK_X_PRIME_2: &str = indoc! {r#"from starkware.cairo.common.cairo_secp.secp256r1_utils import SECP256R1_P
from starkware.cairo.common.cairo_secp.secp_utils import pack
value = pack(ids.x, PRIME) % SECP256R1_P"#};

pub const WRITE_DIVMOD_SEGMENT: &str = indoc! {r#"from starkware.starknet.core.os.data_availability.bls_utils import BLS_PRIME, pack, split
...
pub const CALCULATE_VALUE_2: &str = indoc! {r#"from starkware.cairo.common.cairo_secp.secp256r1_utils import SECP256R1_P
from starkware.cairo.common.cairo_secp.secp_utils import pack
...
pub const COMPUTE_Q_MOD_PRIME: &str = indoc! {r#"from starkware.cairo.common.cairo_secp.secp256r1_utils import SECP256R1_P
from starkware.cairo.common.cairo_secp.secp_utils import pack
...
pub const CREATE_COMMON_ARGS: &str = indoc! {r#"from starkware.cairo.common.patricia_utils import canonic, patricia_guess_descents
...
pub const COMPUTE_SLOPE_2: &str = indoc! {r#"from starkware.cairo.common.cairo_secp.secp256r1_utils import SECP256R1_P
from starkware.cairo.common.cairo_secp.secp_utils import pack
